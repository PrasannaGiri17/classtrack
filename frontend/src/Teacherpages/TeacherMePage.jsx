...
const TeacherMePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch Routine to get live assigned classes
                try {
                    const routine = await timetableService.getTeacherRoutine(targetId);
                    ...
                    console.error("Failed to fetch routine for classes:", routineErr);
                }
    ...
    const confirmPhotoChange = async () => {
        ...
        try {
            ...
            if (!targetId && teacher?._id) {
                targetId = teacher._id;
            }

            if (!targetId) {
                toast({ type: 'error', message: "Teacher ID not found. Cannot update photo." });
                setIsUploading(false);
                return;
            }

            // Construct payload with existing data plus the new profile photo
            const payload = {
                ...
                primarySubject: (teacher.primarySubject && typeof teacher.primarySubject === 'object') ? teacher.primarySubject.subjectName : teacher.primarySubject,
                secondarySubject: (teacher.secondarySubject && typeof teacher.secondarySubject === 'object') ? teacher.secondarySubject.subjectName : teacher.secondarySubject,
                assignedGrades: teacher.assignedGrades?.map(g => String(g.gradeNumber || g)) || [],
                profilePhoto: pendingPhoto,
            };

            const response = await teacherService.updateTeacher(targetId, payload);
                        ...
                        <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8">
                            <div className="relative group">
                                <div className="w-36 h-36 md:w-40 md:h-40 rounded-[36px] bg-[#0b1220] p-1.5 shadow-xl border border-white/10 relative overflow-hidden">
                                    <div
                                        className="w-full h-full rounded-[30px] overflow-hidden bg-slate-800 relative flex items-center justify-center cursor-pointer group/photo"
