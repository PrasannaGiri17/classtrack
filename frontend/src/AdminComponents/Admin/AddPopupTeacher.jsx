      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-[32px] shadow-2xl w-[92vw] max-w-screen-lg border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 slide-in-from-top-4 duration-300 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input name="email" type="email" required onChange={handleChange} value={formData.email} placeholder="faculty@example.com" className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-800/50 border-none rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 text-sm font-bold dark:text-white shadow-inner" />
                </div>
              </div>
            </div>
          </div>
          <div className="flex items-center justify-end gap-3 pt-4 border-t dark:border-slate-800">
            ...
            <button type="submit" disabled={isSubmitting} className="px-8 py-2.5 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-all font-bold shadow-lg shadow-emerald-500/20 active:scale-95">{isSubmitting ? '...' : (teacherToEdit ? 'Update' : 'Enroll Teacher')}</button>
          </div>
        </form>
      </div>
    </div>,
