...
const RoutineView = ({ schoolHours, onUpdateHours, classRoutines, onUpdateRoutines, gradeList }) => {
  ...
  const getMetrics = () => {
    ...
    const used = (classRoutines[selectedGrade] || []).reduce((acc, s) => acc + s.durationMinutes, 0);
  ...
  const handleAddSlot = () => {
    const slots = classRoutines[selectedGrade] || [];
    ...
    onUpdateRoutines(selectedGrade, updated);
  ...
  const calculateTime = (index, isEnd = false) => {
    ...
    const slots = classRoutines[selectedGrade] || [];
    for (let i = 0; i < (isEnd ? index + 1 : index); i++) mins += slots[i].durationMinutes;
    ...
    <div className="space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={toggleLock} className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all shadow-lg active:scale-95 ${isLocked ? 'bg-amber-500 text-white shadow-amber-500/20' : 'bg-white dark:bg-slate-800 text-slate-300 hover:text-emerald-600 border border-slate-200 dark:border-slate-700'}`}>
        ...
        <div className="md:col-span-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 space-y-1.5">
              ...
              <input type="time" value={schoolHours.start} onChange={(e) => onUpdateHours({ ...schoolHours, start: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-800 border-none px-3 py-2 rounded-lg text-sm font-semibold outline-none dark:text-white" />
            </div>
            ...
            <div className="flex-1 space-y-1.5">
              ...
              <input type="time" value={schoolHours.end} onChange={(e) => onUpdateHours({ ...schoolHours, end: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-800 border-none px-3 py-2 rounded-lg text-sm font-semibold outline-none dark:text-white" />
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-2 text-slate-400 uppercase font-bold text-[11px] tracking-widest"><History size={14} /> Sequence Framework</div>
        <div className="space-y-3">
          {(classRoutines[selectedGrade] || []).map((slot, index) => (
            <div key={slot.id} className="group flex items-center gap-6 p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm hover:border-emerald-500/20 transition-all">
              ...
              <button onClick={() => onUpdateRoutines(selectedGrade, classRoutines[selectedGrade].filter(s => s.id !== slot.id))} className="h-9 w-9 flex items-center justify-center text-slate-300 hover:text-red-500 transition-all"><Trash2 size={16} /></button>
            </div>
          ))}
          {(!classRoutines[selectedGrade] || classRoutines[selectedGrade].length === 0) && (
            <div className="py-20 flex flex-col items-center text-center bg-slate-50 dark:bg-slate-900/50 rounded-3xl border border-dashed dark:border-slate-800"><Clock className="text-slate-200 mb-3" size={40} /><p className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Framework Empty</p></div>
          )}
        </div>
      </div>
      ...
      <div className="pt-8 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-6">
        ...
        <button disabled={!metrics.isValid} className={`h-12 px-10 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg transition-all ${metrics.isValid ? 'bg-emerald-600 text-white hover:bg-emerald-700 hover:scale-[1.02] active:scale-95 shadow-emerald-500/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed'}`}>Finalize Framework</button>
      </div>
